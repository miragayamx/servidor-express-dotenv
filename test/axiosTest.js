...
const before = async () => {
  try {
    ...
    const productoExists = productos.data.filter(
      (el) => el.title === productoTest.title
    );
    if (!!productoExists.length) {
      await axios.delete(`${baseURL}/borrar/${productoExists[0]._id}`);
    }
    const productoUpdatedExists = productos.data.filter(
      (el) => el.title === `${productoTest.title} UPDATED`
    );
...
const postProducto = async () => {
  try {
    ...
    const response = await axios.post(`${baseURL}/guardar`, body, {
      headers: {
...
const putProducto = async () => {
  try {
    ...
    const response = await axios.put(
      `${baseURL}/actualizar/${productoExists[0]._id}`,
      body,
      {
        headers: {
          accept: "application/json",
...
const deleteProducto = async (title) => {
  try {
    ...
    const response = await axios.delete(
      `${baseURL}/borrar/${productoExists[0]._id}`
    );
    console.log("DELETE");
    console.log(response.data);
  } catch (err) {
